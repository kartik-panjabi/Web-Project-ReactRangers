...
export default function TenantLeasesPage() {
    ...
    const handleAction = (lease: Lease) => {
        ...
        } else if (lease.advancedPayment?.status === 'pending') {
            router.push(`/dashboard/tenant/leases/${lease._id}/payment`);
        } else if (lease.status === 'active') {
            router.push(`/dashboard/tenant/leases/${lease._id}`);
        }
    };
                        ...
                        <div className="mt-4 flex justify-end">
                            ...
                            >
                                {lease.status === 'pending_tenant_signature' ? 'Sign Lease' :
                                 lease.advancedPayment?.status === 'pending' ? 'Make Payment' :
                                 'View Details'}
                            </button>
                        </div>
                    </div>
